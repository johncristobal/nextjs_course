...
const ProductAdminPage:FC<Props> = ({ product }) => {
    ...
    useEffect(() => {
        const susc = watch(( value, {name, type} ) => {
            if (name === 'title'){
    ...
    const onDeleteTag = ( tag: string ) => {
        const current = getValues('tags').filter( t => t !== tag);
    ...
    const onChangeSize = (size: string) => {
        const currentS = getValues('sizes');
        if (currentS.includes( size )){
            return setValue('sizes', currentS.filter( s => s !== size), {shouldValidate: true});
        }

        setValue('sizes', [...currentS, size], {shouldValidate: true})
    }
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={ 6 }>
                        ...
                        <TextField
                            ...
                            { ...register('price', {
                                required: 'Este campo es requerido',
                                minLength: { value: 0, message: 'Mínimo 0' }
                            })}
                            error={ !!errors.price }
